...
//	public Vector3 BottomLeft;
	...
	void Start()
	{
		if(OverlayObject) //checks if overlayobject exsists and sets distanceTocamera value
		{
...
//			Vector3 vUp = TopLeft - BottomLeft;
			...
			if(manager.IsUserDetected())
			{
				...
				{
					Vector3 posJoint = manager.GetRawSkeletonJointPos(userId, iJointIndex); //set the joint relative to the kinect registered joint position
...
//						Vector3 vPosOverlay = backgroundImage.transform.TransformPoint(localPos);
						...
						if(OverlayObject)
						{
							Vector3 vPosOverlay = Camera.main.ViewportToWorldPoint(new Vector3(scaleX, scaleY, posJoint.z*4));
